...
<div>
  {/* Header Section */}
  <div id="data" className="bg-yellow-900 py-4">
    ...
    <div className="col-span-4">
      ...
      <div className="space-y-1 mt-2 px-4">
        {[
          ...
          ), title: "Phone", value: candidate.phone ? candidate.phone.phone_number : "N/A" },
          {    icon: (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
              <path d="M1.5 8.67v8.58a3 3 0 0 0 3 3h15a3 3 0 0 0 3-3V8.67l-8.928 5.493a3 3 0 0 1-3.144 0L1.5 8.67Z" />
              <path d="M22.5 6.908V6.75a3 3 0 0 0-3-3h-15a3 3 0 0 0-3 3v.158l9.714 5.978a1.5 1.5 0 0 0 1.572 0L22.5 6.908Z" />
            </svg>
          ), title: "Email", value: candidate.applicant_profile?.[0]?.email || "N/A" },
          {   icon: (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
          ...
          ), title: "Gender", value: candidate.applicant_profile?.[0]?.gender_name || "N/A" },
        ].map((item, index) => (
          ...
          {candidate?.education?.length > 0 ? (
            candidate.education.map((item, index) => (
              <div key={index} className="mb-4">
                <p>
                  <span className="font-bold">{item?.course?.course_name || 'Course Name Not Available'}</span>
                </p>
                <p>
                  <span className="font-bold">{item?.level?.education_level || 'Education Level Not Available'}</span>,{' '}
                  {item?.college?.college_name || 'College Name Not Available'}
                </p>
                <p>
        ...
        <div className="mt-2 px-4">
          ...
          <ul className="list-disc list-inside">
            {candidate?.culture?.map((item, index) => (
              <li key={index}>{item?.culture?.culture_name || 'Culture Name Not Available'}</li>
            ))}
          </ul>
          ...
          <ul className="list-disc list-inside">
            {candidate?.software?.map((item, index) => (
              <li key={index}>{item?.software?.software_name || 'Software Name Not Available'}</li>
            ))}
          </ul>
          ...
          <ul className="list-disc list-inside">
            {candidate?.tools?.map((item, index) => (
              <li key={index}>{item?.tool?.tool_name || 'Tool Name Not Available'}</li>
            ))}
          </ul>
          ...
          <ul className="list-disc list-inside">
            {candidate?.knowledge?.map((item, index) => (
              <li key={index}>{item?.knowledge?.knowledge_name || 'Knowledge Name Not Available'}</li>
            ))}
          </ul>
        </div>
      </div>
          ...
          experiences.map((item, index) => (
            <div key={index} className="flex flex-col sm:flex-row mb-4">
              <div className="w-full sm:w-12/12">
                <p className="font-bold">{item?.employer?.employer_name || 'Employer Name Not Available'}</p>
                {item?.positions?.map((position, index) => (
                  <div key={index} className="flex space-x-2 mt-2">
                    ...
                    <div>
                      <p className="text-primary">{position?.position?.position_name || 'Position Name Not Available'}</p>
                      <p>
                        {item?.employer?.region?.region_name || 'Region Not Available'}, {item?.employer?.sub_location || 'Sub-location Not Available'}
                      </p>
                      <p>
                        {position?.start_date ? new Date(position.start_date).getFullYear() : 'N/A'} -{' '}
                        {position?.end_date ? new Date(position.end_date).getFullYear() : 'Present'}
                      </p>
                      <p>{position?.responsibility || 'No responsibility'}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2">
          {candidate?.proficiency?.map((item, index) => (
            <div key={index} className="flex flex-col sm:flex-row mb-4">
              <div className="w-full sm:w-12/12">
                <p className="font-bold">{item?.award || 'Award Not Available'}</p>
                <p className="text-primary">{item?.organization?.organization_name || 'Organization Not Available'}</p>
                <p>
                  <span className="font-bold">Proficiency:</span> {item?.proficiency?.proficiency_name || 'Proficiency Not Available'}
                </p>
                <p className="font-bold">
                  {moment(item?.started).format("YYYY-MM")} - {moment(item?.ended || 'Present').format("YYYY-MM")}
                </p>
              </div>
            </div>
          ))}
      ...
      <div className="mt-4">
        ...
        <div className="mt-2">
          {candidate?.training?.map((item, index) => (
            <div key={index} className="flex flex-col sm:flex-row mb-4">
              <div className="w-full sm:w-12/12">
                <p className="text-primary">{item?.institution || 'Unknown Institution'}</p>
                <p>
                  <span className="font-bold">Training:</span> {item?.name || 'Unknown Training'}
                </p>
                <p className="font-bold">
