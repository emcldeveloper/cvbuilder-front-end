  ...
  <div className="max-w-4xl mx-auto bg-white p-8 rounded-md ">
    ...
    <div className="flex items-center mb-6 mt-4">
      ...
      <div className="w-16 h-16 bg-blue-500 rounded-full flex items-center justify-center text-white text-2xl font-bold">
        {candidate.applicant_profile[0]?.first_name.charAt(0).toUpperCase()}{candidate.applicant_profile[0]?.last_name.charAt(0).toUpperCase()}
      </div>
      {/* Name and Contact Details */}
      <div className="ml-4">
        <h1 className="text-2xl font-bold text-blue-900"> {candidate.applicant_profile[0]?.first_name} {candidate.applicant_profile[0]?.last_name}</h1>
        <p className="text-gray-600">{candidate.phone?.phone_number} | {candidate.applicant_profile[0]?.email }</p>
        {candidate.address.map((item, index) => (
            <p className="text-gray-600">{item.region_name},{item.name}</p> 
          ))}
