 ...
 <div>
  <div className="px-4 sm:px-12 pt-8 pb-12">
    ...
    <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
      ...
      <div className="absolute text-gray-200 opacity-10 text-4xl sm:text-6xl font-bold">Ekazi</div>
    </div>

    {/* Header */}
    <div className="max-w-4xl mx-auto bg-white p-4 sm:p-8 rounded-md">
      <div className="flex flex-col items-center justify-center">
        <h1 className="text-xl sm:text-2xl font-bold" style={{ color: 'rgb(46, 88, 166)' }}>CURRICULUM VITAE</h1>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center mb-6 mt-4">
        ...
        <div className="ml-0 sm:ml-4 mt-4 sm:mt-0 text-center sm:text-left">
          ...
          {candidate?.address?.length > 0 ? (
            ...
            <p className="text-sm sm:text-base text-gray-600">Address Not Available</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-sm sm:text-base text-gray-700">{candidate.careers[0]?.career || 'Not Specified'}</p>
      </div>
            ...
            <ul className="flex flex-col list-disc list-inside space-x-0">
              {candidate?.culture?.length > 0 ? (
                candidate.culture.map((item, index) => (
                  <li key={index} className="py-0 text-sm sm:text-base">{item?.culture?.culture_name || 'Culture not specified'}</li>
                ))
              ) : (
                <li className="py-0 text-sm sm:text-base">No culture information available</li>
              )}
            </ul>

            {/* Software Section */}
            <h1 className="font-bold mt-2 text-sm sm:text-base">Software</h1>
            <ul className="flex flex-col list-disc list-inside space-x-0">
              {candidate?.software?.length > 0 ? (
                candidate.software.map((item, index) => (
                  <li key={index} className="py-0 text-sm sm:text-base">{item?.software?.software_name || 'Software not specified'}</li>
                ))
              ) : (
                <li className="py-0 text-sm sm:text-base">No software information available</li>
              )}
            </ul>
            ...
            <ul className="flex flex-col list-disc list-inside space-x-0">
              {candidate?.tools?.length > 0 ? (
                candidate.tools.map((item, index) => (
                  <li key={index} className="py-0 text-sm sm:text-base">{item?.tool?.tool_name || 'Tool not specified'}</li>
                ))
              ) : (
                <li className="py-0 text-sm sm:text-base">No tools information available</li>
              )}
            </ul>

            {/* Skills & Knowledge Section */}
            <h1 className="font-bold mt-2 text-sm sm:text-base">Skills & Knowledge</h1>
            <ul className="flex flex-col list-disc list-inside space-x-0">
              {candidate?.knowledge?.length > 0 ? (
                candidate.knowledge.map((item, index) => (
                  <li key={index} className="py-0 text-sm sm:text-base">{item?.knowledge?.knowledge_name || 'Knowledge not specified'}</li>
                ))
              ) : (
                <li className="py-0 text-sm sm:text-base">No skills & knowledge information available</li>
              )}
            </ul>
          </div>
              ...
              <ul className="flex flex-col list-disc list-inside space-x-0">
                {candidate?.language?.length > 0 ? (
                  candidate.language.map((item, index) => (
                    <li key={index} className="py-0 text-sm sm:text-base">{item?.language?.language_name || 'Language not specified'}</li>
                  ))
                ) : (
                  <li className="py-0 text-sm sm:text-base">No language information available</li>
                )}
              </ul>
            </div>
          </div>
        ...
        <div className="col-span-12 sm:col-span-8">
          ...
          <div>
            ...
            {experiences?.length > 0 ? (
              experiences.map((item, index) => (
                <div key={index} className="flex flex-col sm:flex-row mb-4">
                  <div className="w-full sm:w-4/12">
                    <div>
                      <span className="capitalize text-sm sm:text-base" style={{ color: 'rgb(46, 88, 166)' }}>
                  ...
                  <div className="w-full sm:w-8/12">
                    <div>
                      <p className="font-bold text-sm sm:text-base">{item?.employer?.employer_name || 'Employer name not available'}</p>
                    </div>
                    ...
                    <div className="ml-0 mt-2">
                      {item?.positions?.length > 0 ? (
                        item.positions.map((position, index) => (
                          <div key={index} className="flex space-x-2">
                            ...
                            <div className="py-0 my-0">
                              <p className="text-primary py-0 my-0 text-sm sm:text-base">{position?.position?.position_name || 'Position name not available'}</p>
                              <i className="text-sm sm:text-base">{position?.employer?.employer_name || 'Employer name not available'}</i>
                        ...
                        <p className="text-sm sm:text-base">No positions available</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <p className="text-sm sm:text-base">No work experience available</p>
            )}
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap mt-2">
              {candidate?.education?.length > 0 ? (
                ...
                <p className="text-sm sm:text-base">No education information available</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            {candidate?.proficiency?.length > 0 ? (
              candidate.proficiency.map((item, index) => (
                <div key={index} className="flex flex-col sm:flex-row mb-4">
                  ...
                  <div className="w-full sm:w-8/12">
                    ...
                    <p className="text-primary text-sm sm:text-base">{item?.organization?.organization_name || 'Organization name not available'}</p>
              ...
              <p className="text-sm sm:text-base">No proficiency information available</p>
            )}
          </div>
            ...
            {candidate?.training?.length > 0 ? (
              candidate.training.map((item, index) => (
                <div key={index} className="flex flex-col sm:flex-row mb-4">
                  ...
                  <div className="w-full sm:w-8/12">
                    <p className="text-primary-500 text-sm sm:text-base">{item?.institution || 'Institution not available'}</p>
                    <p className="flex space-x-2 text-sm sm:text-base">
              ...
              <p className="text-sm sm:text-base">No training information available</p>
            )}
          </div>
        </div>
      </div>
