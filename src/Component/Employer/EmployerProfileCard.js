...
const EmployerProfileCard = ({ client, isAuthenticated }) => {
  ...
  return (
    <>
      ...
      {/* Modals */}
      <JoinModal show={showJoin} onHide={() => setShowJoin(false)} client={client} />
      <MessageModal show={showMessage} onHide={() => setShowMessage(false)} clientId={client.id} />
      <RateModal show={showRate} onHide={() => setShowRate(false)} clientId={client.id} />
    </>
  );
};
