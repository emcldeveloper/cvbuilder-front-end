...
const SalaryEstimation = () => {
    ...
    useEffect(() => {
        const sum = allowances.reduce((total, amount) => total + parseFloat(amount || 0), 0);
    ...
    }, [basicPay, netPay, totalAllowance, salaryType, exchangeRate, currencyType]);
    ...
    const calculatePayAmount = (taxableAmount) => {
        ...
        if (currencyType === 'usd' && exchangeRate > 0) {
            ...
            if (taxableAmount > payoneReduction && taxableAmount <= paytwoReduction) {
                payAmount = (taxableAmount - payoneReduction) * taxData.payone_percentage;
            } else if (taxableAmount > paytwoReduction && taxableAmount <= paythreeReduction) {
                payAmount = (taxableAmount - paytwoReduction) * taxData.paytwo_percentage + paytwoAddition;
            } else if (taxableAmount > paythreeReduction && taxableAmount <= payfourReduction) {
                payAmount = (taxableAmount - paythreeReduction) * taxData.paythree_percentage + paythreeAddition;
            } else if (taxableAmount > payfourReduction) {
                payAmount = (taxableAmount - payfourReduction) * taxData.payfour_percentage + payfourAddition;
            }
        } else {
            if (taxableAmount > taxData.payone_reduction && taxableAmount <= taxData.paytwo_reduction) {
                payAmount = (taxableAmount - taxData.payone_reduction) * taxData.payone_percentage;
            } else if (taxableAmount > taxData.paytwo_reduction && taxableAmount <= taxData.paythree_reduction) {
                payAmount = (taxableAmount - taxData.paytwo_reduction) * taxData.paytwo_percentage + taxData.paytwo_addition;
            } else if (taxableAmount > taxData.paythree_reduction && taxableAmount <= taxData.payfour_reduction) {
                payAmount = (taxableAmount - taxData.paythree_reduction) * taxData.paythree_percentage + taxData.paythree_addition;
            } else if (taxableAmount > taxData.payfour_reduction) {
                payAmount = (taxableAmount - taxData.payfour_reduction) * taxData.payfour_percentage + taxData.payfour_addition;
            }
        }
    ...
    const estimateGrossSalary = (targetNetPay) => {
        ...
        } while (Math.abs(calculatedNetPay - targetNetPay) > tolerance && iteration < maxIterations);
    ...
    return (
        <div className="py-2">
                    ...
                    <Form style={{ fontSize: '13px', }}>
                        ...
                        <Form.Group style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3px' }}>
                            <Form.Label style={{ flex: 1, fontWeight: 600 }}>
                                {salaryType === 'net' ? 'Net Salary' : 'Basic Salary'} ({currencyType === 'usd' ? 'USD' : 'TZS'})
                            </Form.Label>
                            <Form.Control
                                type="number"
                                ...
                                onChange={(e) =>
                                    salaryType === 'net'
                                        ? setNetPay(e.target.value > 100000000000 ? '' : e.target.value)
                                        : setBasicPay(e.target.value > 100000000000 ? '' : e.target.value)
                                }
            ...
            {/* Tax Rate Modal */}
            <Modal show={showTaxModal} onHide={() => setShowTaxModal(false)} size="lg">
                ...
                <Modal.Body>
                    ...
                    <h6 className="fw-bold mt-4"> <strong>Additional Deductions:</strong></h6>
                    <p><strong>SSF (Social Security Fund):</strong> 20% of an employee's gross salary,
                        where the employee contributes 10% of their salary, and the employer contributes 10% of the
                        employee's gross salary to the Social Security Fund.</p>
                    <p><strong>WCF (Workers Compensation Fund):</strong> Employers contribute 0.5%
                        of the employee's salary for workplace injury insurance.</p>
                    <p>
                        <strong> SDL (Skills Development Levy):</strong> is a
                        tax imposed on employers to fund workforce training and skills development programs.
                        The applicable rate for SDL is 3.5% of the total emoluments paid to all employees during the
                        month.
                        Employees include permanent employees, part-time employees, secondary employees, casual
                        laborers, etc.
                    </p>
                </Modal.Body>
                <Modal.Footer>
