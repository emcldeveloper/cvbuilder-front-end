    ...
    return (
        ...
        >
            <Modal.Header closeButton className="border-bottom-0 pb-0">
                <Modal.Title style={{ fontSize: 'clamp(14px, 3vw, 18px)', textAlign: 'center' }}>
                        ...
                        <div className="d-flex">
                            ...
                            <div>
                                ...
                                <p className="mb-0 small">
                                    Your profile is {completecheck?.sections ? Math.round((completecheck.sections.length - (missingFields?.length || 0)) / completecheck.sections.length * 100) : 0}% complete.
                                    You must complete {missingFields?.length || 0} essential {missingFields?.length > 1 ? 'sections' : 'section'} before applying.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ minWidth: '120px' }}>
                        <div
                            style={{
                                ...
                                height: '80px',
                                borderRadius: '50%',
                                background: `conic-gradient(#2E58A6 ${completecheck?.sections ? (completecheck.sections.length - (missingFields?.length || 0)) / completecheck.sections.length * 100 : 0}%, #f0f0f0 ${completecheck?.sections ? (completecheck.sections.length - (missingFields?.length || 0)) / completecheck.sections.length * 100 : 0}%)`,
                                display: 'flex',
                                alignItems: 'center',
                        ...
                        >
                            ...
                            >
                                {completecheck?.sections
                                    ? `${Math.round(
                                        ((completecheck.sections.length - (missingFields?.length || 0)) /
                                ...
                                <div className="flex-grow-1">
                                    <div className="d-flex flex-column flex-md-row justify-content-between align-items-start">
                                        <span className="mb-1 mb-md-0">
                                            <i className="fas fa-plus-circle text-danger me-2" style={{ fontSize: '1.1rem' }}></i>
                                            <strong className="text-dark">
                                                {(completecheck?.sections && completecheck.sections[`applicant_${field?.toLowerCase()}`]) || field}:
                                            </strong>
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ...
                                <p>Form or input for <strong>{currentField}</strong> goes here.</p>
                            </div>
