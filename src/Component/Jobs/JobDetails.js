...
import Checkcompleteprofile from '../Profile/Checkcomplete';
// import Checkcompleteprofile from '../Profile/Checkcomplete';
...
const JobDetails = ({ job }) => {
  ...
  console.log('job url', job);
  const [loading, setLoading] = useState(false);
  const [completecheck, setjobcompletecheck] = useState(false);
  ...
  const handleApply = () => {
    ...
    // First check if profile is complete using the Checkcompleteprofile component's logic
    if (!profileComplete) {
    ...
    if (job.job_email) {
      navigate("/jobseeker/apply-job-email");
    } else if (job.job_external_url) {
      window.open(job.job_external_url?.external_url, "_blank");
    } else {
