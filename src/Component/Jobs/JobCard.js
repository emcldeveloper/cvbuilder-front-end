...
// Slugify utility for generating clean URLs
const slugify = (text) =>
  text
    ?.toString()
    .toLowerCase()
...
const JobCard = ({ job }) => {
  ...
  state={{ job }} // ✅ Pass the job object here
  style={{
    color: '#2E58A6',
    textDecoration: 'none',
    display: 'block',
  }}
  aria-label={`Show job: ${jobTitle}`}
>
  Show
</Link>
            </Col>
