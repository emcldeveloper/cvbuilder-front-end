...
const Language = ({   label, onSelect, onOptionsLoad, initialValue }) => {
  ...
  const fetchOptions = async () => {
    try {
      const response = await fetch('https://test.ekazi.co.tz/api/applicant/language');
      const data = await response.json();

      // Format options for react-select
      const formattedOptions = Array.isArray(data.language)
      ? data.language.map(option => ({
 ...
 const loadOptions = (inputValue, callback) => {
  ...
  return (
    ...
    <AsyncSelect
        ...
        placeholder=" language" // Placeholder text
