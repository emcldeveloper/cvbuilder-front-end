...
const UnderstandLanguage = ({   label, onSelect, onOptionsLoad, initialValue }) => {
  ...
  const fetchOptions = async () => {
    try {
      const response = await fetch('https://test.ekazi.co.tz/api/applicant/language_understand');
      const data = await response.json();

      // Format options for react-select
      const formattedOptions = Array.isArray(data.understand_ability)
      ? data.understand_ability.map(option => ({
          // value: option.education_level,
          value: option.id,
          label: option.understand_ability,
        }))
      : [];
 ...
 const loadOptions = (inputValue, callback) => {
  ...
  return (
    ...
    <AsyncSelect
        ...
        placeholder=" read ability" // Placeholder text
...
export default UnderstandLanguage;
