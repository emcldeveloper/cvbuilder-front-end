...
const PositionLevel = ({   label, onSelect, onOptionsLoad, initialValue }) => {
  ...
  const fetchOptions = async () => {
    try {
      ...
      const formattedOptions = Array.isArray(data.position_level)
      ? data.position_level.map(option => ({
      ...
      console.log('check position level  is their',formattedOptions);
 ...
 const loadOptions = (inputValue, callback) => {
  ...
  return (
    ...
    <AsyncSelect
        ...
        placeholder=" level" // Placeholder text
