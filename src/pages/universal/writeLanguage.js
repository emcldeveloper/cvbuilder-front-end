...
const WriteLanguage = ({   label, onSelect, onOptionsLoad, initialValue }) => {
  ...
  const fetchOptions = async () => {
    try {
      const response = await fetch('https://test.ekazi.co.tz/api/applicant/language_write');
      const data = await response.json();

      // Format options for react-select
      const formattedOptions = Array.isArray(data.language_write)
      ? data.language_write.map(option => ({
          // value: option.education_level,
          value: option.id,
          label: option.write_ability,
        }))
      : [];
 ...
 const loadOptions = (inputValue, callback) => {
  ...
  return (
    ...
    <AsyncSelect
        ...
        placeholder=" Write ability" // Placeholder text
