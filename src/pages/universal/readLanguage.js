...
const ReadLanguage = ({   label, onSelect, onOptionsLoad, initialValue }) => {
  ...
  const fetchOptions = async () => {
    try {
      ...
      const formattedOptions = Array.isArray(data.language_read)
      ? data.language_read.map(option => ({
          // value: option.education_level,
          value: option.id,
          label: option.read_ability,
        }))
      : [];
