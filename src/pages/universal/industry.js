...
const Employers = ({ label, onSelect,onOptionsLoad, initialValue  }) => {
  ...
  const fetchOptions = async () => {
    try {
      const response = await fetch('https://test.ekazi.co.tz/api/applicant/industry');
      const data = await response.json();

      // Format options for react-select
      const formattedOptions = Array.isArray(data.industry)
      ? data.industry.map(option => ({
  ...
  const loadOptions = (inputValue, callback) => {
    const filteredOptions = options.filter(option => {
      // Ensure `option.label` and `inputValue` are strings
