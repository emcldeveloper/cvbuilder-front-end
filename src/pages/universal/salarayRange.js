...
const SalaryRange = ({ label, onSelect ,onOptionsLoad, initialValue }) => {
  ...
  const fetchOptions = async () => {
    try {
      const response = await fetch('https://test.ekazi.co.tz/api/applicant/salary_range');
      const data = await response.json();

      // Format options for react-select
      const formattedOptions = Array.isArray(data.salary_range)
      ? data.salary_range.map(option => ({
          value: option.id,
          label: option.low,
        }))
      : [];
 ...
 fetchOptions();
  ...
  return (
    ...
    <AsyncSelect
        ...
        placeholder="salary" // Placeholder text
