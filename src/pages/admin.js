      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-bold">
            {templates.find(t => t.id.toString() === selectedTemplate)?.name} Preview
          </h2>
          <button 
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="border rounded-lg p-4 mb-4">
            {templates.find(t => t.id.toString() === selectedTemplate)?.component}
          </div>
          
          <div className="bg-gray-50 p-4 rounded-lg">
            <HideInfo uuid={uuid} template={selectedTemplate} />
            <p className="text-gray-600 mb-4">
              Your CV will be saved to your account and available for future downloads.
            </p>
            <input
              type="text"
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {currentTemplates.map((tpl) => (
                            <div
                                ...
                                className={`cursor-pointer p-4 rounded-lg border-2 transition-all ${selectedTemplate === tpl.id.toString() ? 'border-primary bg-primary/10' : 'border-gray-200 hover:border-gray-300'}`}
                            >
                        ...
                        <div className="flex justify-center mt-6 gap-1">
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(number => (
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-full ${currentPage === number ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700'}`}
                                >
                                    {number}
                                </button>
                            ))}
