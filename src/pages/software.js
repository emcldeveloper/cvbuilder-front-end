...
const Software = ({ label, onSelect, onOptionsLoad, initialValue }) => {
    ...
    const fetchOptions = async () => {
        try {
            const response = await fetch('https://test.ekazi.co.tz/api/applicant/software');
            const data = await response.json();

            // Format options for react-select
            const formattedOptions = Array.isArray(data.software)
                ? data.software.map(option => ({
                    value: option.id,
    ...
    return (
      ...
      <AsyncCreatableSelect
        ...
        placeholder="Select a Software" // Placeholder text
