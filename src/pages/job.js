...
import { FaUsers, FaBriefcase, FaMoneyBillAlt, FaCalendar } from 'react-icons/fa';
...
const JobListing = () => {
  ...
  const fetchJobs = async (pageNum) => {
    try {
      ...
      // https://ekazi.co.tz/api/applicant/featuredJobs
  ...
  return (
    <div className="job-listing-container">
      ...
      {!loading && !hasMore && <div className="no-more-jobs">No more jobs to load</div>}
  ...
  return (
    <div className="job-card">
      ...
      <div className="job-card-content">
        <h3 className="job-title">{job.job_position?.position_name || 'No Position'} ({job.quantity})</h3>
        <p className="company-name">{job.client?.client_name || 'No Company'}</p>
        <div className="job-details">

          <p><span className="detail-label">Type:</span> {job.job_type?.type_name || 'N/A'}</p>
          <p><span className="detail-label">Location:</span> {job.country?.name || 'N/A'} : {job.region?.region_name || 'N/A'}</p>
          <p><span className="detail-label-deadline text-orange-500">Deadline:</span> {new Date(job.dead_line).toLocaleDateString()}</p>
          <p><span className="detail-label">Industry:</span> {job.industry?.industry_name || 'N/A'}</p>
      ...
      <div class="card-footer bg-white" >
        <div class="d-flex justify-content-between ">
          ...
          <div class="p-2"><a onClick={() => onViewDetails(job)} >Show</a></div>

          <div className="detail-label-view"> <i class="fa fa-eye" className="font-view  fa fa-eye"> {job.statistic?.job_views || 0}</i>
          </div>
          <div class="p-2 detail-label-view "><i class="fa fa-heart" title="Please Sign In"> 0</i>
          </div>
        </div>
      </div>
    </div>
...
`}</style>
          <div className="stats-container">
            <div className="stats-row">
              ...
              <div className="stat-item">
                ...
                <div className="stat-value">{job.statistic?.job_views || 0}</div>
              </div>
              ...
              <div className="stat-item">
                <div className="stat-icon">
                  <FaBriefcase style={{ color: '#D36314', fontSize: '1.5rem' }} />
                </div>
                <div className="stat-label">Job Type</div>
                <div className="stat-value">{job.job_type?.type_name || 'N/A'}</div>
              </div>

              {/* Salary */}
              <div className="stat-item">
                <div className="stat-icon">
                  <FaMoneyBillAlt style={{ color: '#D36314', fontSize: '1.5rem' }} />
                </div>
                ...
                <div className="stat-value">
                  {job.salaryRange ?
                    ...
                    (job.entry_salary > 0 || job.exit_salary > 0) ?
                      `${job.entry_salary || ''} ${job.entry_salary && job.exit_salary ? '-' : ''} 
       ${job.exit_salary || ''} ${job.currency_id === 3 ? 'USD' : ''}`.trim()
              ...
              <div className="stat-item">
                <div className="stat-icon">
                  <FaCalendar style={{ color: '#D36314', fontSize: '1.5rem' }} />
                </div>
                <div className="stat-label">Deadline</div>
                <div className="stat-value">
                  {new Date(job.dead_line) >= new Date() ?
                    new Date(job.dead_line).toLocaleDateString('en-US', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }) :
                    <span style={{ color: 'red' }}>Expired: {new Date(job.dead_line).toLocaleDateString()}</span>}
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Company/Title Row */}
          <div className="job-header" style={{ display: 'flex', alignItems: 'center', gap: '20px', padding: '15px' }}>
            {/* Company Logo */}
            <div style={{ flexShrink: 0, width: '120px', height: '75px' }}>
              <img
                src={`https://ekazi.co.tz/${job.client?.logo || 'img/default.png'}`}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px', flexWrap: 'wrap' }}>
                <h5 className="job-title" style={{ margin: 0 }}>
                  ...
                  {job.quantity > 0 && <span style={{ marginLeft: '5px' }}>({job.quantity})</span>}
                </h5>
              ...
              <p className="company-name" style={{ margin: '5px 0 0' }}>
                <span style={{ fontWeight: 'bold' }}>Company:</span> {job.show_client_name === 0 ? job.client?.client_name : 'Exact Manpower Consulting Ltd'}
              </p>
            </div>
          </div>
          ...
          {job.job_report_to && (
            <>
              <div className="reporting-structure" style={{ marginBottom: '1rem' }}>
                <h6 className="fw-bold main-colur" style={{ marginBottom: '1rem' }}>Reporting Structure</h6>

                <div className="reporting-grid" style={{
                  display: 'grid',
                  ...
                  {job.job_report_to.report_to && (
                    <>
                      <div className="fw-bold" style={{ whiteSpace: 'nowrap' }}>Report To:</div>
                  ...
                  {job.job_report_to.supervises && (
                    <>
                      <div className="fw-bold" style={{ whiteSpace: 'nowrap' }}>Supervision:</div>
                  ...
                  {job.job_report_to.interacts_with && (
                    <>
                      <div className="fw-bold" style={{ whiteSpace: 'nowrap' }}>Interact:</div>
          ...
          <h6 className="fw-bold main-colur" style={{ marginBottom: '1rem' }}>Job Requirements</h6>
              ...
              <div className="reporting-structure" style={{ marginBottom: '1rem' }}>
                ...
                <div className="reporting-grid" style={{
                  ...
                  <>
                    ...
                    <div className="fw-bold" style={{ whiteSpace: 'nowrap' }}>Education:</div>
                    <div  >
                      {job.job_education.map((item, index) => (
                        <span key={index}>
                          {index > 0 ? ', ' : ''}
                          {item.education_level?.education_level} {item.course?.course_name}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="col-md-9">{job.position_level?.position_name}</div>


                    <div className="col-md-3 fw-bold">Gender:</div>
                    <div className="col-md-9">{job.job_gender?.gender_name}</div>
                    {job.salaryRange && (
                      <>
          ...
          <div className="row mb-3">
            <div className="col-md-12">
              ...
              <div dangerouslySetInnerHTML={{ __html: job.job_duties?.main_duties || 'No duties provided' }} />
            </div>
          </div>
          ...
          <div className="row mb-3">
            <div className="col-md-12">
              ...
              <div dangerouslySetInnerHTML={{ __html: job.job_other_requirement?.other_requirement || 'No requirements provided' }} />
            </div>
          </div>
          ...
          <div className="row">
            <div className="col-md-12">
              <p>
                ...
                <br />
                <span className="fw-bold">Industry:</span> {job.industry?.industry_name}<br />
                <span className="fw-bold">Company:</span> {job.show_client_name === 0 ? job.client?.client_name : 'Exact Manpower Consulting Ltd'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="modal-footer mt-3">
          ...
          >
            ...
            {job?.email ? (
              <Link
                to={new Date(job.dead_line) >= new Date()
                  ? `/email/job/apply/${job.id}`
                  : "#"}
                onClick={() => new Date(job.dead_line) >= new Date() && localStorage.setItem('jobId', job.id)}
                style={{ color: 'white', textDecoration: 'none' }}
              >
                {new Date(job.dead_line) >= new Date() ? 'Apply Now' : 'Expired'}
              </Link>
              ...
              onClick={(e) => {
                ...
                const allowedDomains = ['yourdomain.com', 'yourotherdomain.com'];
                
                if (allowedDomains.includes(url.hostname)) {
                  // If it's an allowed domain, proceed with navigation
                  window.open(job.job_external_url.external_url, '_blank', 'noopener,noreferrer');
                } else {
                  window.open(job.job_external_url.external_url, '_blank', 'noopener,noreferrer');
