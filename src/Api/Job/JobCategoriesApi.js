...
export const getJobCountByRegion = async () => {
  ...
  if (cachedData) {
    console.log('Returning cached data for job count by region');
...
export const getJobCategorySummary = async () => {
  ...
  const cachedData = getCache(cacheKey);
  if (cachedData) {
    console.log('Returning cached data for job category summary');
    return cachedData; // Return cached data if available
  }
...
export const getClientsJobCountByIndustry = async () => {
  ...
  const cachedData = getCache(cacheKey);
  if (cachedData) {
    console.log('Returning cached data for clients job count by industry');
    return cachedData; // Return cached data if available
  }
